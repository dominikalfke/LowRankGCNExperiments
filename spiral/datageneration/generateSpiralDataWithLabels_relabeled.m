function [data, label, run] = generateSpiralDataWithLabels_relabeled(Nc,Ns,h,r,option)
...
%% Improvement: 
% relabel each point to its closest Mu to avoid points that
% are impossible to classify correctly

for i = 1:Nc*Ns
    [~, label(i)] = min(arrayfun(@(j) sum((data(i,:) - Mu(j,:)).^2), 1:Nc));
end
